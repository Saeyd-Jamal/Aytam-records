(function ($) {
    ...
    $("#filterError-btn").click(function(){
        $("#filter-error-div").slideToggle();
    });

    $('#error-filter').on("change", function (e) {
        let type_error = $(this).val();
        $.ajax({
            url: "/salah1/orphans_records/filterError/", //data-id
            method: "GET",
            data: {
            ...
            success: function (response) {
                ...
                if (response['data'].length != 0) {
                    ...
                    $("#table_orphan").append(
                        "<tr><td colspan='3'>لا توجد بيانات خاطئة</td></tr>"
                    );
                }
            },
        });
